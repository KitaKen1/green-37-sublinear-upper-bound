...
/-!
# Green 37: standalone Lean4Web proof

A self-contained copy of the relevant Formal Conjectures definitions, followed by
a proof of the registered little-o and big-O answers `N ↦ N`.
-/
...
def ContainsAP (A : Set ℕ) (k : ℕ) (d : ℕ) : Prop :=
  ∃ a, ∃ s, s ⊆ A ∧ IsAPOfLengthWith s (k : ℕ∞) a d

def IsAPCover (A : Set ℕ) (N k : ℕ) : Prop :=
  ∀ d, 1 ≤ d ∧ d ≤ N → ContainsAP A k d

noncomputable def m (N k : ℕ) : ℕ :=
  sInf {r | ∃ A : Finset ℕ, A.card = r ∧ IsAPCover (A : Set ℕ) N k}

lemma containsAP_of_mem {A : Set ℕ} {k d a : ℕ} (hd : 0 < d)
    (hmem : ∀ j < k, a + j * d ∈ A) : ContainsAP A k d := by
  ...
  · constructor
    · calc
        ENat.card (Set.range f) = ENat.card (Fin k) := (ENat.card_congr (Equiv.ofInjective f hf)).symm
        _ = k := by simp
    · ext x
...
lemma periodicCover_isAPCover (t k N : ℕ) :
    IsAPCover (periodicCover t k N : Set ℕ) N k := by
  rintro d ⟨hd, hdN⟩
...
lemma m_le_periodicCover_card (t k N : ℕ) :
    m N k ≤ (periodicCover t k N).card := by
  apply Nat.sInf_le
...
theorem m_isLittleO_id (k : ℕ) :
  ...
  have hdecomp :
    ...
    ring
  rw [hdecomp] at hm
  have hfinal : (m N k : ℝ) ≤ c * (N : ℝ) := by
    calc
      (m N k : ℝ) ≤ C + (k : ℝ) ^ 2 / Nat.fermatNumber t * (N : ℝ) := hm
      _ ≤ c / 2 * (N : ℝ) + c / 2 * (N : ℝ) := add_le_add hCN hcoefN
      _ = c * (N : ℝ) := by ring
  simpa [Real.norm_eq_abs, abs_of_nonneg] using hfinal

theorem m_isBigO_id (k : ℕ) :
    (fun N ↦ (m N k : ℝ)) =O[atTop] (fun N ↦ (N : ℝ)) :=
  (m_isLittleO_id k).isBigO


/-- The exact little-o target registered in Formal Conjectures, with answer `N ↦ N`. -/
theorem green_37_littleO (k : ℕ) :
    (fun N ↦ (m N k : ℝ)) =o[atTop]
      (answer(fun N : ℕ ↦ (N : ℝ)) : ℕ → ℝ) :=
  m_isLittleO_id k

/-- The exact big-O target registered in Formal Conjectures, with answer `N ↦ N`. -/
...
#check Green37.green_37_littleO
#check Green37.green_37_bigO
#print axioms Green37.green_37_littleO
#print axioms Green37.green_37_bigO
